...
function TextSummarizer() {
  ...
  const handleTextSummarization = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:4002/summarize-text/v2",
        {
          text: inputText,
        }
      );
  ...
  const handleTextHistory = async () => {
    try {
      const response = await axios.get("http://localhost:4002/api-data"); // Fetching history data
  ...
  const handleClearHistory = async () => {
    try {
      setLoading(true);
      await axios.delete("http://localhost:4002/clear-history");
      setLoading(false);
